...
def accounts_login_view(request):
    email = EmailMessage('Subject', 'Body', to=['cstarrfish27@gmail.com'], from_email='christianjstarr@icloud.com')
    email.send()
    print('Sent Email')
    if request.method == 'POST':
...
def accounts_signup_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            email = EmailMessage(mail_subject, message, to=[email], from_email='christianjstarr@icloud.com')
            email.send()
            user.save()
